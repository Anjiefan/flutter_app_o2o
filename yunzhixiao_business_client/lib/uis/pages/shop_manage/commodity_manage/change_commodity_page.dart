...
import 'package:yunzhixiao_business_client/view_model/shop_manage/commodity/commodity_edit_model.dart';

class ChangeCommodityPage extends StatefulWidget {
  final int commodityId;

  ChangeCommodityPage({Key key, this.commodityId}) : super(key: key);

  _ChangeCommodityState createState() => _ChangeCommodityState();
}

class _ChangeCommodityState extends State<ChangeCommodityPage> {
  TextEditingController nameTextEditingController = TextEditingController();
  TextEditingController typeTextEditingController = TextEditingController();
  TextEditingController priceTextEditingController = TextEditingController();
  var _imageFile;
  CommodityEditModel globalModel;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          actions: <Widget>[
            InkWell(
              ...
              onTap: () {
                globalModel.deleteCommodity(context).then((value) {
                  if (value) {
              ...
              onTap: () {
                if (_imageFile != null) {
                  ImageUtils.putFile(_imageFile, (value) async {
                    ...
                    globalModel.updateCommodity(context).then((value) {
                      if (value) {
                        Navigator.pop(context);
                      }
                    });
                  });
                } else if (globalModel.commodity.img != null) {
                  if (globalModel.commodity.name.trim() == "" ||
                      globalModel.commodity.price == null ||
                      globalModel.commodity.price <=0) {
                    showToast("名称不能为空或价格输入格式不合法");
                    return;
                  }
                  globalModel.updateCommodity(context).then((value) {
        ...
        body: ProviderWidget<CommodityEditModel>(
            model: CommodityEditModel(widget.commodityId),
            onModelReady: (model) {},
            builder:
                (BuildContext context, CommodityEditModel model, Widget child) {
              if (model.busy) {
