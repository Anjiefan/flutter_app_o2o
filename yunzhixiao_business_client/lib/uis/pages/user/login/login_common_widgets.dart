...
import 'package:yunzhixiao_business_client/commons/routers/app_router.dart';
import 'package:yunzhixiao_business_client/service/user_repository.dart';
import 'package:yunzhixiao_business_client/uis/widgets/button_progress_indicator_widget.dart';
                ...
                if (type == 0) {
                  model
                    ...
                    if (value) {
                      ...
                      model.setIdle();
                      int status = StorageManager.sharedPreferences
                          .getInt(Config.VERIFYING_STATUS);
                      if (status == -1 || status == 0) {
                        Navigator.pushReplacementNamed(
                            context, RouteName.registerVerify);
                      }
                    }
                  }).catchError((e){
                    model.setIdle();
                      int status = StorageManager.sharedPreferences.getInt(Config.VERIFYING_STATUS);
                      if (status == -1 || status == 0) {
...
//                  .pushReplacementNamed(RouteName.registerVerify);
              //TODO 正式环境替换恢复正常逻辑
              //正常环境代码开始
              var formState = Form.of(context);
              if (formState.validate()) {
